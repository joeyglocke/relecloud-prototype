...
// (- / *), blockquotes (> ...), and links ([text](url)). Hard-line-breaks are
// preserved within a paragraph; blank lines split paragraphs.
//
// Deliberately not pulling in `react-markdown` — this is ~80 lines, no extra
...
export default function Markdown({ source }) {
  ...
  return (
    <div className="md-content">
      {blocks.map((b, i) => {
        if (b.type === 'h1') return <h2 key={i} className="md-h md-h1">{renderInline(b.text, `h${i}`)}</h2>
        if (b.type === 'h2') return <h3 key={i} className="md-h md-h2">{renderInline(b.text, `h${i}`)}</h3>
        if (b.type === 'h3') return <h4 key={i} className="md-h md-h3">{renderInline(b.text, `h${i}`)}</h4>
        if (b.type === 'p') {
          return (
            <p key={i} className="md-p">
              {b.lines.flatMap((l, j) => {
                const inline = renderInline(l, `p${i}-${j}`)
        ...
        if (b.type === 'ul') {
          return (
            <ul key={i} className="md-ul">
              {b.items.map((it, j) => (
                <li key={j} className="md-li">{renderInline(it, `li${i}-${j}`)}</li>
              ))}
            </ul>
          )
        }
        if (b.type === 'bq') {
          return (
            <blockquote key={i} className="md-blockquote">
              {b.lines.map((l, j) => (
                <span key={j}>{j > 0 && <br />}{renderInline(l, `bq${i}-${j}`)}</span>
              ))}
            </blockquote>
          )
        }
